...
class MyForm(baseclass, formclass):
    def __init__(self, parent=None):
        ...
        if filehandle:
            ...
            save_list = [8, 8, 0, datetime.datetime.now(), 0,False]
    ...
    def stopTimer(self):
        self.stopTime = not self.stopTime
        if self.stopTime:
            self.stopBtn.setText("start Time")
        if not self.stopTime:
            self.stopBtn.setText("stop Time")
...
# event handler for the 'tick' timer (every 5 seconds)
    ...
    def closeEvent(self, event):
        ...
        save_list = [self.happiness, self.health, self.hunger, datetime.datetime.now(), self.time_cycle,self.stopTime]
